...
trait InteractsWithDockerComposeServices
{
    ...
    protected function buildDockerCompose(array $services, string $host = '')
    {
        ...
        // Ensure the new host is set...
        if (! array_key_exists($host, $compose['services']) && array_key_exists('laravel.test', $compose['services'])) {
            $compose['services'][$host] = $compose['services']['laravel.test'];
            unset($compose['services']['laravel.test']);
        }

        // Adds the new services as dependencies of the host service...
        if (! array_key_exists($host, $compose['services'])) {
            $this->warn('Couldn\'t find the ' . ($host ?? 'host') . ' service. Make sure you add ['.implode(',', $services).'] to the depends_on config.');
        } else {
            $compose['services'][$host]['depends_on'] = collect($compose['services'][$host]['depends_on'] ?? [])
                ->merge($services)
                ->unique()
                ->values()
                ->all();
        ...
        collect($services)
            ->filter(function ($service) {
                return in_array($service, ['mysql', 'pgsql', 'mariadb', 'mongodb', 'redis', 'valkey', 'meilisearch', 'typesense', 'minio', 'elasticsearch', 'rustfs', 'rabbitmq']);
            })->filter(function ($service) use ($compose) {
